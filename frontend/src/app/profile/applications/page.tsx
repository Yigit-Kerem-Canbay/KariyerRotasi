...
export default function MyApplicationsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const { data } = await api.get<{ items: AppRow[] }>(
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-4">
      ...
      {items.length === 0 ? (
        <p className="text-slate-600">Henüz başvuru yok.</p>
      ) : (
            ...
            <li key={row.id}>
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="shrink-0 text-xs font-bold uppercase tracking-wide px-2 py-1 rounded-lg bg-slate-100 text-slate-700">
