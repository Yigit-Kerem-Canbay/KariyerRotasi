/**
 * public/logos altında gerçekten bulunan dosyalar — sıralama ve gösterim tek kaynak.
 */
...
/** İsimden tahmin — yalnızca diskte dosyası olan anahtarlar */
...
export function companyLogoSrc(key: string): string {
  return `/logos/${key}.png`;
}

const warmed = new Set<string>();

/** Tarayıcı önbelleğine logo çek — liste/detay geçişlerinde gecikmeyi azaltır */
export function warmupCompanyLogo(key: string | null | undefined): void {
  if (typeof window === 'undefined' || !key || !LOGO_FILE_KEYS.has(key)) return;
  const src = companyLogoSrc(key);
...
export function warmupCompanyLogoFor(company: {
  name: string;
  website?: string | null;
}): void {
  warmupCompanyLogo(resolveLogoFileKey(company));
}
